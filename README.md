...
- Developer API 详情可独立展示，并会显示新 API 返回的外部资源入口；旧站通过 Cloudflare 验证后再补充可直接入队的下载直链与社交数据
- 全屏截图查看器，支持左右箭头与键盘导航
...
- 未完成旧站 Cloudflare 验证时，详情页会先显示 Developer API 可提供的介绍、截图、PV、标签、评分和外部资源入口；评论、云端收藏以及可直接入队的下载直链仍需旧站访问可用后补齐
