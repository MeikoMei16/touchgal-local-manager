...
const scanForGalgameFolders = (rootPaths) => {
  const results = [];
  rootPaths.forEach((rootPath) => {
    if (!fs$1.existsSync(rootPath)) return;
    const dirs = fs$1.readdirSync(rootPath, { withFileTypes: true });
    dirs.forEach((dir) => {
      if (dir.isDirectory()) {
        const fullPath = path$1.join(rootPath, dir.name);
        const tgIdPath = path$1.join(fullPath, ".tg_id");
        if (fs$1.existsSync(tgIdPath)) {
          const id = fs$1.readFileSync(tgIdPath, "utf8").trim();
          results.push({
            path: fullPath,
            folderName: dir.name,
            tg_id: id
...
ipcMain.handle("scan-local-library", (_event, paths) => {
  return scanForGalgameFolders(paths);
});
ipcMain.handle("tag-folder", (_event, folderPath, id) => {
  const tgIdPath = path$1.join(folderPath, ".tg_id");
  try {
    fs$1.writeFileSync(tgIdPath, id, "utf8");
    return { success: true };
  } catch (err) {
