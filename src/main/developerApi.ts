...
const stripMarkdownMedia = (markdown: string) =>
  markdown
    ...
    .replace(/^\s*\[[^\]]+]\(\s*<?(?:https?:\/\/)?[^)\s>]*(?:youtube\.com|youtu\.be|bilibili\.com|player\.bilibili\.com|\.mp4(?:\?|$)|\.webm(?:\?|$)|\.ogg(?:\?|$)|\.mov(?:\?|$)|\.m3u8(?:\?|$)|\.flv(?:\?|$))[^)\s>]*>?(?:\s+["'][^"']*["'])?\s*\)\s*$/gim, '')
    .replace(/^\s*<img[^>]+src=["'][^"']+["'][^>]*>\s*$/gim, '')
    .replace(/^\s*https?:\/\/[^\s"'<>）)]+\.(?:avif|gif|jpe?g|png|webp)(?:[?#][^\s"'<>）)]*)?\s*$/gim, '')
    .replace(/^\s*https?:\/\/[^\s"'<>）)]*(?:youtube\.com|youtu\.be|bilibili\.com|player\.bilibili\.com|\.mp4(?:\?|$)|\.webm(?:\?|$)|\.ogg(?:\?|$)|\.mov(?:\?|$)|\.m3u8(?:\?|$)|\.flv(?:\?|$))[^\s"'<>）)]*\s*$/gim, '')
