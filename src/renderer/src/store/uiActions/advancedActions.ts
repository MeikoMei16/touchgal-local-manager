                  ...
                  const next = ds.resources.map((item: AdvancedResourceRecord) =>
                    item.uniqueId === resource.uniqueId
                      ? {
                          ...
                          companyAliases: intro.companyAliases?.length ? intro.companyAliases : (item as any).companyAliases,
                          platform: intro.platform || item.platform,
                          language: intro.language || item.language,
                          type: intro.type?.length ? intro.type : item.type,
                          releasedDate: intro.releasedDate || item.releasedDate,
                          resourceUpdateTime: intro.resourceUpdateTime ?? item.resourceUpdateTime,
                          touchgalUrl: intro.touchgalUrl ?? item.touchgalUrl,
