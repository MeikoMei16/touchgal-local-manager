...
const normalizeTouchGalResourceInput = (value: unknown) => {
  ...
  return {
    ...
    ratingCount: raw.ratingCount ?? raw.rating_count ?? (ratingSummary as any)?.count ?? ratingStat.count ?? 0,
    introduction: raw.introduction ?? null,
    screenshots: raw.screenshots ?? raw.fullScreenshotUrls ?? raw.full_screenshot_urls ?? [],
    pvUrl: raw.pvUrl ?? raw.pv_url ?? raw.pvVideoUrl ?? raw.pv_video_url ?? null,
    ratingSummary,
