      ...
      <div className={contentClassName}>
        <section className="overflow-hidden rounded-[2.25rem] border border-slate-200 bg-white shadow-[0_24px_70px_-40px_rgba(15,23,42,0.35)]">
          <div className="grid gap-0 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="relative min-h-[320px] overflow-hidden bg-slate-100">
              {game.banner_url ? (
                <img src={game.banner_url} alt={game.name ?? '本地游戏'} className="h-full w-full object-cover" />
              ) : (
            ...
            <div className="flex flex-col gap-5 p-6 md:p-8">
              <div>
                <div className="text-[11px] font-black uppercase tracking-[0.24em] text-slate-400">模板弹层</div>
                <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-950">{game.name ?? `本地游戏 ${game.id}`}</h1>
              ...
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-[1.4rem] border border-slate-200 bg-slate-50 p-4">
                  <div className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">状态</div>
                  <div className="mt-2 text-base font-black text-slate-900">{game.status}</div>
                </div>
                <div className="rounded-[1.4rem] border border-slate-200 bg-slate-50 p-4">
                  <div className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">浏览</div>
                  <div className="mt-2 text-base font-black text-slate-900">{formatCompactNumber(game.view_count)}</div>
                </div>
                <div className="rounded-[1.4rem] border border-slate-200 bg-slate-50 p-4">
                  <div className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400">下载</div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="text-[11px] font-black uppercase tracking-[0.24em] text-slate-400">本地上下文</div>
            <div className="mt-5 space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4">
                  <div className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">来源</div>
                  <div className="mt-2 text-sm font-black text-slate-900">{game.source}</div>
                </div>
                <div className="rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4">
                  <div className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">关联时间</div>
                  <div className="mt-2 text-sm font-black text-slate-900">{formatDate(game.linked_at)}</div>
                </div>
                <div className="rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4">
                  <div className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">唯一 ID</div>
                  <div className="mt-2 text-sm font-black text-slate-900">{game.unique_id ?? '未关联'}</div>
                </div>
                <div className="rounded-[1.25rem] border border-slate-200 bg-slate-50 p-4">
                  <div className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">可执行程序</div>
          ...
          <div className="rounded-[2rem] border border-dashed border-slate-300 bg-white/85 p-6 shadow-sm">
            ...
            <div className="mt-4 text-2xl font-black tracking-tight text-slate-900">重型本地管理区</div>
